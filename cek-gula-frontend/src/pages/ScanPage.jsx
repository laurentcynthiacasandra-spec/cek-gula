...
function ScanPage() {
  ...
  const [aiResult, setAiResult] = useState(null) // hasil prediksi AI
  ...
  const handleFileChange = async (e) => {
    ...
    // Langsung prediksi ke AI API setelah foto dipilih
    setPredicting(true)
    try {
  ...
  const handleAnalyze = async () => {
    ...
    if (!namaFinal) {
      setError('AI belum mendeteksi makanan. Upload foto terlebih dahulu.')
      return
    }
    ...
    try {
      // Cari nutrisi dari dataset backend
      const nutrisiResult = await searchNutrisi(namaFinal.trim())

      let kadarGula = 100
      if (nutrisiResult.data.length > 0) {
        kadarGula = nutrisiResult.data[0].kadar_gula_estimasi
      }

      // Simpan ke scan_results
      const result = await createGula({
        nama:      namaFinal,
        kadarGula,
        imageFile: image,
      })
      ...
      // Gabungkan rekomendasi AI ke hasil
      const hasilFinal = {
        ...result.result,
        nutrisiDetail: {
          ...result.result.nutrisiDetail,
          rekomendasi: aiResult?.rekomendasi || result.result.nutrisiDetail?.rekomendasi,
        },
      ...
      <div className="flex flex-col items-center justify-center flex-1 px-4 py-12">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 max-w-md">
            Upload foto jajananmu, biar <span className="text-green-700 font-semibold">AI kami</span> yang deteksi dan hitung nutrisinya otomatis!
          </p>
        </div>

        {/* Error */}
        {error && (
          <div className="w-full max-w-md mb-4 bg-red-50 border-l-4 border-red-500 text-red-700 text-sm px-4 py-3 rounded-lg">
            {error}
          </div>
        )}
        ...
        {aiResult && !predicting && (
          <div className="w-full max-w-md mb-4 bg-green-50 border border-green-200 rounded-xl px-5 py-4">
            <p className="text-green-800 font-bold text-sm mb-1">
              🤖 AI Mendeteksi: <span className="capitalize">{aiResult.prediction}</span>
            </p>
            <p className="text-green-600 text-xs">
              Tingkat keyakinan: <span className="font-bold">{aiResult.confidence?.toFixed(1)}%</span>
            </p>
            {aiResult.rekomendasi && (
              <p className="text-green-700 text-xs mt-2 italic">{aiResult.rekomendasi}</p>
            )}
          </div>
        )}
        ...
        {/* Upload Box */}
        <div
          onClick={() => !scanning && !predicting && fileInputRef.current.click()}
          className="w-full max-w-md rounded-2xl overflow-hidden shadow-md cursor-pointer relative"
          style={{ height: '320px' }}>
            ...
            <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center rounded-2xl">
              ...
              <div className="mt-24 text-center">
                <p className="text-white font-bold text-lg">
                  {predicting ? 'Mendeteksi makanan...' : 'Menyimpan data...'}
                </p>
        ...
        <div className="mt-4 bg-green-50 border border-green-200 rounded-xl px-5 py-3 max-w-md w-full">
          <p className="text-green-700 text-sm">
            💡 <span className="font-semibold">Tips:</span> Pastikan foto jajanan terlihat jelas agar AI bisa mendeteksi dengan akurat.
          </p>
        </div>

        {/* Tombol Analisis — muncul setelah AI selesai prediksi */}
        {aiResult && !scanning && !predicting && (
          <button
            onClick={handleAnalyze}
            className="mt-6 bg-green-700 text-white px-10 py-4 rounded-xl text-lg font-semibold hover:bg-green-800 active:scale-95 transition shadow-lg flex items-center gap-3">
            <FaUpload size={18} /> Simpan & Lihat Hasil
          </button>
        )}

        {/* Tombol upload kalau belum ada foto */}
        {!image && !predicting && (
          <button
            onClick={() => fileInputRef.current.click()}
            className="mt-6 border-2 border-green-700 text-green-700 px-10 py-4 rounded-xl text-lg font-semibold hover:bg-green-50 transition flex items-center gap-3">
