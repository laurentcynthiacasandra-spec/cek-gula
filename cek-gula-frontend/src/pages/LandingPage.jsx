    ...
    <div className="min-h-screen bg-[#f0f7f0] font-sans">
      ...
      <nav className="bg-white px-6 py-4 flex justify-between items-center shadow-sm sticky top-0 z-50">
        ...
        <div className="desktop-menu flex items-center gap-8">
          <span onClick={() => scrollTo('hero')} className="text-green-600 font-semibold cursor-pointer">Beranda</span>
          ...
          <button onClick={() => navigate('/login')} className="bg-green-700 text-white px-6 py-2 rounded-lg font-semibold hover:bg-green-800 transition">Masuk</button>
        </div>
        <button onClick={() => setMenuOpen(!menuOpen)} className="hamburger-btn flex-col gap-1.5 p-2">
      ...
      {menuOpen && (
        <div className="mobile-menu bg-white shadow-md px-6 py-4 flex-col gap-4 sticky top-16 z-40">
          <span onClick={() => scrollTo('hero')} className="block py-2 text-green-600 font-semibold cursor-pointer">Beranda</span>
          <span onClick={() => scrollTo('fitur')} className="block py-2 text-gray-600 cursor-pointer">Fitur</span>
          <span onClick={() => scrollTo('tentang')} className="block py-2 text-gray-600 cursor-pointer">Tentang</span>
          <span onClick={() => scrollTo('artikel')} className="block py-2 text-gray-600 cursor-pointer">Artikel</span>
          <button onClick={() => navigate('/login')} className="w-full bg-green-700 text-white px-6 py-2 rounded-lg font-semibold mt-2">Masuk</button>
        </div>
      )}

      <div id="hero" className="flex items-center justify-between px-16 py-20 max-w-6xl mx-auto">
        <div className="flex-1 pr-8">
          <span className="bg-green-100 text-green-700 text-sm font-semibold px-4 py-1 rounded-full mb-6 inline-block">🌿 Powered by AI</span>
          ...
          <p className="text-gray-600 text-lg italic mb-8 leading-relaxed">
            Scan jajanan pasar Indonesia dan dapatkan informasi kandungan gizi serta estimasi beban glikemiknya secara instan.
          </p>
          <div className="flex gap-4 hero-buttons">
            <button onClick={() => navigate('/register')} className="bg-green-700 text-white px-8 py-4 rounded-xl text-lg font-semibold hover:bg-green-800 flex items-center gap-3 transition shadow-lg">
              Mulai Gratis <MdArrowForward size={22} />
            </button>
            <button onClick={() => scrollTo('fitur')} className="border-2 border-green-700 text-green-700 px-8 py-4 rounded-xl text-lg font-semibold hover:bg-green-50 transition">
              Pelajari Fitur
            </button>
          </div>
        </div>
        <div className="flex-1 flex justify-center hero-image">
          <div className="relative">
            ...
            <img src={heroImg} alt="Hero" className="relative z-10 w-80 h-80 object-cover rounded-full shadow-xl" />
          </div>
        </div>
      </div>
      ...
      <div className="bg-green-700 py-12 px-16">
        <div className="max-w-6xl mx-auto grid grid-cols-3 gap-8 text-center text-white">
          <div><p className="text-4xl font-black mb-2">50+</p><p className="text-green-200 text-sm">Jenis Jajanan Pasar</p></div>
          <div><p className="text-4xl font-black mb-2">85%+</p><p className="text-green-200 text-sm">Akurasi Deteksi AI</p></div>
          <div><p className="text-4xl font-black mb-2">100%</p><p className="text-green-200 text-sm">Gratis Digunakan</p></div>
        </div>
      </div>
      ...
      <div id="fitur" className="py-20 px-16 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-8 fitur-grid">
          <div className="bg-white rounded-2xl p-8 shadow-sm hover:shadow-md transition">
            <div className="bg-green-100 w-14 h-14 rounded-2xl flex items-center justify-center mb-6"><FaRobot size={28} className="text-green-700" /></div>
          ...
          <div className="bg-white rounded-2xl p-8 shadow-sm hover:shadow-md transition">
            <div className="bg-green-100 w-14 h-14 rounded-2xl flex items-center justify-center mb-6"><FaMagnifyingGlassChart size={28} className="text-green-700" /></div>
          ...
          <div className="bg-white rounded-2xl p-8 shadow-sm hover:shadow-md transition">
            <div className="bg-green-100 w-14 h-14 rounded-2xl flex items-center justify-center mb-6"><FaChartLine size={28} className="text-green-700" /></div>
      ...
      <div id="tentang" className="bg-white py-20 px-16">
        <div className="max-w-6xl mx-auto flex items-center gap-16 tentang-wrapper">
          <div className="flex-1">
            <div className="bg-green-50 rounded-3xl p-10">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white rounded-xl p-4 text-center shadow-sm"><p className="text-green-700 font-black text-2xl">Indonesia</p><p className="text-gray-400 text-xs mt-1">Peringkat 5 Dunia Diabetes</p></div>
                <div className="bg-white rounded-xl p-4 text-center shadow-sm"><p className="text-green-700 font-black text-2xl">AI</p><p className="text-gray-400 text-xs mt-1">Teknologi Terdepan</p></div>
              </div>
            </div>
          </div>
          <div className="flex-1">
            ...
            <h2 className="text-4xl font-black text-gray-900 mb-6 leading-tight">Mengapa Cek-Gula Hadir?</h2>
            <p className="text-gray-500 leading-relaxed mb-6">Indonesia menempati peringkat ke-5 dunia dalam kasus diabetes. Salah satu pemicunya adalah konsumsi jajanan pasar yang tinggi gula tanpa informasi nutrisi yang jelas.</p>
            <p className="text-gray-500 leading-relaxed mb-8">Cek-Gula hadir sebagai solusi berbasis AI untuk membantu masyarakat Indonesia membuat pilihan makanan yang lebih cerdas dan sehat setiap harinya.</p>
            <button onClick={() => navigate('/register')} className="bg-green-700 text-white px-8 py-3 rounded-xl font-semibold hover:bg-green-800 transition flex items-center gap-2">Coba Sekarang <MdArrowForward size={20} /></button>
          </div>
        </div>
      </div>
      ...
      <div id="artikel" className="py-20 px-16 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-8 artikel-grid">
          {[
            { img: artikel1, tag: 'Glikemik', title: 'Apa itu Indeks Glikemik?', desc: 'Pelajari bagaimana indeks glikemik mempengaruhi kadar gula darah dan energi harianmu.', date: '28 April 2026' },
            { img: artikel2, tag: 'Diabetes', title: 'Bahaya Sugar Crash di Tempat Kerja', desc: 'Sugar crash bisa menurunkan produktivitas secara drastis. Kenali gejalanya dan cara mencegahnya.', date: '25 April 2026' },
            { img: artikel3, tag: 'Nutrisi', title: 'Jajanan Pasar yang Aman untuk Diabetesi', desc: 'Tidak semua jajanan pasar berbahaya. Ini daftar jajanan yang relatif aman dikonsumsi.', date: '20 April 2026' }
          ].map((artikel, i) => (
            <div key={i} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition cursor-pointer">
              <div className="h-48 overflow-hidden"><img src={artikel.img} alt={artikel.title} className="w-full h-full object-cover hover:scale-105 transition duration-300" /></div>
              <div className="p-6">
      ...
      <div className="bg-green-700 py-20 px-16 text-center">
        ...
        <button onClick={() => navigate('/register')} className="bg-white text-green-700 px-10 py-4 rounded-xl text-lg font-bold hover:bg-green-50 transition shadow-lg">Daftar Gratis Sekarang</button>
      </div>

      <footer className="bg-gray-900 text-white py-12 px-16">
        <div className="max-w-6xl mx-auto flex justify-between items-start footer-wrapper">
          <div>
            <div className="flex items-center gap-3 mb-4">
              <div className="bg-green-700 text-white w-10 h-10 rounded-lg flex items-center justify-center text-xl">🍃</div>
              <span className="font-bold text-xl">Cek-Gula</span>
            </div>
            <p className="text-gray-400 text-sm max-w-xs leading-relaxed">Solusi cerdas berbasis AI untuk deteksi kandungan gizi jajanan pasar Indonesia</p>
